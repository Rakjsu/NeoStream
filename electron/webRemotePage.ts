 ...
 *
 * Two views: "Controle" (transport buttons) and "Guia" — a second screen that
 * lists the live channels with the now/next EPG of the playing channel and
 * lets you tap a channel to switch (the LiveTV page pushes the guide data).
 */
...
<script>
  (function () {
    ...
    function renderGuide() {
      ...
      var epg = guide.epg;
      document.getElementById('now-ch').textContent = playing ? playing.name : (channels.length ? 'Nenhum canal tocando' : 'Nenhum canal tocando');
    ...
    document.querySelectorAll('button.ctl').forEach(function (btn) {
